        ...
        if file.mimetype not in app.config['file_allowed']:
            ...
            if user_id:
                ...
                if face_id:
                    ...
                    return_output = json.dumps({"status": "success", "id": user_id, "name": name, "face": [face_data]})
...
def recognize():
    if 'file' not in request.files:
        ...
        if file.mimetype not in app.config['file_allowed']:
            ...
            if user_id:
                user = get_user_by_id(user_id)
                message = {"message": "Found {0} matched with your face image".format(user["name"]),
                           "user": user}
                return success_handle(json.dumps(message))
            else:
                return error_handle("Cannot find match for your face image, try another image")
